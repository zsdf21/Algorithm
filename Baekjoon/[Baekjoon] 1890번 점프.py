...
한 번 점프를 할 때, 방향을 바꾸면 안 된다. 즉, 한 칸에서 오른쪽으로 점프를 하거나, 
아래로 점프를 하는 두 경우만 존재한다.

가장 왼쪽 위 칸에서 가장 오른쪽 아래 칸으로 
규칙에 맞게 이동할 수 있는 경로의 개수를 구하는 프로그램을 작성하시오.
...
for i in range(N):
    for j in range(N):
        ...
        # 조건) 경로 존재, 점프할 숫자가 0이 아닌 경우
        if dp[i][j] > 0 and arr[i][j] > 0:
            jump = arr[i][j]        # 점프할 수는 해당 arr에서의 값
            
            if j + jump < N:                # 우측 이동(조건: arr 범위 내)
                dp[i][j+jump] += dp[i][j]   # 해당 jump자리에 i, j값 누적
                
            if i + jump < N:                # 하단 이동
                dp[i+jump][j] += dp[i][j]   # 해당 jump자리에 i, j값 누적
